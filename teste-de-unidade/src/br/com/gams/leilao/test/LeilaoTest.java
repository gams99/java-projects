...
public class LeilaoTest {
    ...
    public void naoDeveAceitarMaisDoQue5LancesDeUmMesmoUsuario(){
        ...
        leilao.propoe(new Lance(steveJobs, 7500));
        leilao.propoe(new Lance(billGates, 7800));

        leilao.propoe(new Lance(steveJobs, 8000));
        leilao.propoe(new Lance(billGates, 8500));

        //deve ser ignorado
        leilao.propoe(new Lance(steveJobs, 9000));

        assertEquals(10, leilao.getLances().size());
        assertEquals(8500.0, leilao.getLances().get(leilao.getLances().size()-1).getValor(), 0.00001);
